...
module.exports = function (config) {
  ...
  if (config.protocol == 'https') {
    if (!config.key || !config.cert) {
      throw new Error('Missing string path for ssl cert and key.');
    }

    client.isTLS = true;
    client.cert = config.cert;
    client.key = config.key;
    client.passphrase = config.passphrase;
  }
  ...
  let buildOpts = function (method, path, opts) {
    ...
    if (client.isTLS) {
      opts.cert = fs.readFileSync(client.cert);
      opts.key = fs.readFileSync(client.key);
      opts.passphrase = client.passphrase;
    }
